...
def detect_gesture(image_path, show_crop=True, margin_ratio=0.5):
    #with crop
    ...
    # # # Take first detected hand to crop and analyze
    ...
    # # # Run landmark detection only on the cropped hand
    # # cropped_result = hands.process(rgb_cropped)

    # # if not cropped_result.multi_hand_landmarks:
    # #     return "no_gesture"

    # # cropped_landmarks = cropped_result.multi_hand_landmarks[0]

    # # # Draw landmarks on cropped image
    # # annotated_crop = cropped_frame.copy()
    # mp_draw.draw_landmarks(rgb_frame, detection_result, mp_hands.HAND_CONNECTIONS)

    # # Gesture detection on cropped landmarks
    # detected_label = "no_gesture"
    # if is_thumbs_up(rgb_frame.landmark):
    #     detected_label = "like"
    # elif is_peace_sign(rgb_frame.landmark):
    #     detected_label = "peace"
    # elif is_rock_n_roll_sign(rgb_frame.landmark):
    #     detected_label = "rock"

    # # Show the result if requested
    # show_crop = False
    # if show_crop:
    #     cv2.putText(rgb_frame, detected_label, (10, 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
    #     cv2.imshow("Cropped Hand with Landmarks and Label", rgb_frame)
    #     cv2.waitKey(0)
    ...
    # Draw landmarks on the original BGR image (not RGB)
    mp_draw.draw_landmarks(frame, hand_landmarks, mp_hands.HAND_CONNECTIONS)

    # Gesture detection on landmarks
    detected_label = "no_gesture"
    if is_thumbs_up(hand_landmarks.landmark):
        detected_label = "like"
    elif is_peace_sign(hand_landmarks.landmark):
        detected_label = "peace"
    elif is_rock_n_roll_sign(hand_landmarks.landmark):
        detected_label = "rock"

    # Optional: Show the frame with landmarks
    show = True
    if(show):
        cv2.imshow("Hand with Landmarks", frame)
        cv2.waitKey(0)
        cv2.destroyAllWindows()
...
# ========== Evaluation over Folders ========== #
# folders = {
#     "like": "./dataset_created/like",
#     "peace": "./dataset_created/peace",
#     "rock": "./dataset_created/rock"
# }
folders = {
    "like": "./from_thesis/one"
}
...
for label, folder in folders.items():
    ...
    for img_file in image_files:
        ...
        predicted = detect_gesture(img_path)
        true_labels.append(label)
        pred_labels.append(predicted)
        folder_preds.append(predicted)

    # Summarize predictions for this folder
    print(f"Folder '{label}': Processed {len(folder_preds)} images")
    unique, counts = np.unique(folder_preds, return_counts=True)
    summary = dict(zip(unique, counts))
    print(f"Predictions summary: {summary}\n")

# ========== Confusion Matrix ========== #
labels = ["like", "peace", "rock", "no_gesture", "Invalid"]
...
plt.ylabel("True")
plt.title("Gesture Detection Confusion Matrix")
plt.show()